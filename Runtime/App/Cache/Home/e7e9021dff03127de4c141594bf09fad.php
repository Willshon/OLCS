    ...
    <?php echo hook('Advs', 'below_sidebar');?>
            ...
            <div class="col-xs-12 col-md-9 main">
                ...
                <section class="tag-info mt30">
                                       <?php if(!empty($cateinfo['img'])): ?><img class="pull-left avatar-32 mr10" src="/cs<?php echo ($cateinfo["path"]); ?>"><?php endif; ?>
                                        <h1 class="h3"><?php echo ($cateinfo["name"]); ?></h1>
...
<?php $map=array($cid,'true','0','1','tj desc,id desc','','10','','','','','',);$data = callApi("Art/getArt",$map);$__LIST__ = $data['data']; if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><section class="stream-list__item">
    <div class="blog-rank">
    ...
    <div class="summary <?php if($vo['img'] != ''): ?>has_post_thumbnail<?php endif; ?>">
        <ul class="author list-inline">
                ...
                <?php echo (friendlyDate($vo["create_time"])); ?>
                     <span class="split"></span>
                 <code><a href="<?php echo CSU('/artlist/'.$vo['cid'],'Index/artlist',array('cid'=>$vo['cid']));?>"><?php echo (get_cate_nameByid($vo["cid"])); ?></a></code>

            </li>
                    </ul>
        <h2 class="title"><a href="<?php echo CSU('/artc/'.$vo['id'],'Index/artc',array('id'=>$vo['id']));?>"><?php echo ($vo["titleicon"]); ?><span><?php echo ($vo["title"]); ?></span></a></h2>
                 <?php if($vo['img'] != ''): ?><div class="entry-thumbnail">
<a href="<?php echo CSU('/artc/'.$vo['id'],'Index/artc',array('id'=>$vo['id']));?>" title="<?php echo ($vo["title"]); ?>">
<img src="<?php echo ($vo["img"]); ?>" alt="<?php echo ($vo["title"]); ?>" ></a>
        ...
        <p class="excerpt wordbreak "><?php echo (cutstr_html(op_t($vo["description"]),150)); ?></p>
         ...
         <?php $map=array($cid,'true','0','1','','','','',);$data = callApi("Art/getArtCount",$map);$count = $data['data'];$__PAGE__ = new \Think\Page($count, 10);echo $__PAGE__->show(); ?>
