...
<div id="page-content">
      ...
      <div class="form-label float-left" >
       <label>组名：</label>
                ...
                <div class="form-input col-md-2">
                    <input type="text" value="<?php echo ($_REQUEST["name"]); ?>" name="name" size="10" />
                </div>
         ...
         <button type="submit" class="btn medium bg-orange"> <span class="button-content">
               ...
               查询
                ...
                <a href="<?php echo U('add');?>" target="dialog"  title="新增管理组" rel="roleadd" width="500px" height="400px" class="btn medium bg-green"><span class="button-content">
          ...
          <div class="dropdown float-right mrg15R">
                                ...
                                <ul class="dropdown-menu float-right">
                                     ...
                                     <li class="hidden">
                                        <a href="/cs/index.php/Admin/Role/outxls/ids/all" target="dwzExport">
                                            <i class="glyph-icon icon-sign-out font-size-13 mrg5R"></i>
                                            <span class="font-bold">导出全部</span>
                                        </a>
                                    </li>
                                     <li class="hidden">
                                        <a href="/cs/index.php/Admin/Role/outxls" target="checkedExport" idname="ids">
                                            <i class="glyph-icon icon-sign-out font-size-13 mrg5R"></i>
 ...
 <table class="table text-left"  id="zstable">
        <thead>
            <tr><th class="nosort" width="30">序号</th>
            <th>组名</th>
				<th>上级组</th>
				<th width="50">状态</th>
				<th width="100">描述</th>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td><?php echo ($i); ?></td>
				...
				<td>
                    <a href="<?php echo U('access',array('groupId'=>$vo['id']));?>" rel="roleaccess" class="btn bg-green small" title="<?php echo ($vo['name']); ?> 权限设置 " ><span class="button-content"><i class="glyph-icon icon-shield"></i> 授权</span> </a>

                    <a href="<?php echo U('edit',array('id'=>$vo['id']));?>" rel="roleedit<?php echo ($vo['id']); ?>" title="编辑<?php echo ($vo['name']); ?>" class="btn bg-blue small" target="dialog"   width="500px" height="400px" ><span class="button-content"><i class="glyph-icon icon-edit"></i> 编辑</span></a>
                     <?php echo (showStatus($vo['status'],$vo['id'])); ?>
                    <a href="<?php echo U('foreverdelete',array('id'=>$vo['id']));?>" class="btn bg-red small" target="ajaxTodo" title="确定要删除该行信息吗？"><span class="button-content"><i class="glyph-icon icon-trash-o"></i> 删除</span></a>
                </td>
