...
<div class="example-code">
  ...
  <button type="submit" class="btn medium bg-orange"> <span class="button-content">
               ...
               查询
                ...
                <a href="<?php echo U('add');?>" target="dialog"  width="440px" height="400px" rel="configadd" title="新增用户" class="btn medium bg-green">
                ...
                <i class="glyph-icon icon-plus"></i> 新增
                            ...
                            <div class="dropdown float-right mrg15R">
                                ...
                                <ul class="dropdown-menu float-right">
                                     ...
                                     <li class="hidden">
                                        <a href="/cs/index.php/Admin/Member/outxls/ids/all" target="dwzExport">
                                            <i class="glyph-icon icon-sign-out font-size-13 mrg5R"></i>
                                            <span class="font-bold">导出全部</span>
                                        </a>
                                    </li>
                                     <li class="hidden">
                                        <a href="/cs/index.php/Admin/Member/outxls" target="checkedExport" idname="ids">
                                            <i class="glyph-icon icon-sign-out font-size-13 mrg5R"></i>
    ...
    <table class="table text-left"  id="zstable">
        <thead>
            <tr>
                ...
                <th class="nosort text-center" width="270">操作</th>
            </tr>
        </thead>
        <tbody>

           <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> <td><?php echo ($vo['uid']); ?></td>
				    <td><?php echo (query_user('username',$vo['uid'])); ?></td>
					<td><?php echo ($vo['nickname']); ?></td>
					<td><?php echo (getMroleNameByUserId($vo['uid'])); ?></td>
					<td><?php echo (getGroupNameByUserId($vo['uid'])); ?></td>
                    ...
                    <td><input type="checkbox" name="ids" class="j-icheck" value="<?php echo ($vo['uid']); ?>"></td>
                <td>
                <div class="mrg0A">
                 <a href="<?php echo U('edit',array('id'=>$vo['uid']));?>" rel="editmember<?php echo ($vo['uid']); ?>"   class="btn small bg-blue" target="dialog"  width="440px" height="360px" ><span class="button-content"><i class="glyph-icon icon-edit"></i> 编辑</span></a>
                    <a href="<?php echo U('password',array('id'=>$vo['uid']));?>"  class="btn small bg-orange" target="dialog"  width="420px" height="200" ><span class="button-content">修改密码</span></a>
                    <?php echo (showStatus($vo['status'],$vo['uid'],CONTROLLER_NAME)); ?>
                    <a href="<?php echo U('foreverdelete',array('id'=>$vo['uid']));?>" class="btn small bg-red" target="ajaxTodo" title="确定要删除该行信息吗？"><span class="button-content"><i class="glyph-icon icon-trash-o"></i> 删除</span></a>
                    </div>
                </td>
