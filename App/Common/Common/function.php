...
function getNameById($tableName,$id,$name='name'){
    $map['id']=$id;
    return M($tableName)->where($map)->getField($name);
}
// 通过课程ID获取教师姓名
function getTNameByCId($cid){
    $tid= M('course')->where(array('id'=>$cid))->getField('teaid');
    return M('teacher')->where(array('id'=>$tid))->getField('name');
}
// 通过用户ID获取专业名称
function getSNameByUId($uid){
    $subid=M('student')->where(array('id'=>$uid))->getField('subid');
    return M('subject')->where(array('id'=>$subid))->getField('name');
}
...
function CSU($str1,$str2,$arg,$flag=false){
