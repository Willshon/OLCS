...
class WeixinController extends HomeController {
    ...
    public function index(){
      ...
      foreach ($catearr as $key =>$vo){
      	...
      	$cate[$key]=array('type'=>'view','url'=>(is_ssl()?'https://':'http://').$_SERVER['HTTP_HOST'].'/'.C('WEB_DIR').'/'.CSU('/artlist/'.$vo['id'],'Index/artlist',array('cid'=>$vo['id'])),'name'=>$vo['name']);
      }
      $newmenu =  array(
      		   		"button"=>
      		   			array(
    ...
    public function wxtext($data){
    	...
    	if ( $lastdo == 'search' ) {
    		...
    		foreach ($sartarr as $key =>$svo){
    			$sart[$key]=array('Title'=>$svo['title'],'Description'=>cutstr_html(op_t($svo['description']),50),'PicUrl'=>getImgs($svo['description'],0),'Url'=>(is_ssl()?'https://':'http://').$_SERVER['HTTP_HOST'].'/'.C('WEB_DIR').'/'.CSU('/artc/'.$svo['id'],'Index/artc',array('id'=>$svo['id'])));
