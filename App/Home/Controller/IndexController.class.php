...
class IndexController extends HomeController{
   ...
   public function gzart(){
    ...
    public function course(){
        ...
        $this->coursed =M('score')->where(array('stuid'=>$uid))->select();
        if(I('name')){
           $map['name']=I('name');
        };
        $this->course =M('course')->where($map)->order('sort')->select();
 ...
 public function focusnum(){
    ...
    public function xuanke(){
        ...
        $status=I('get.status');
        if($status){
            M('score')->data($data)->add();
        }else{
            M('score')->where($data)->delete();
        }

        $this->success('操作成功','Index/course');
    }
